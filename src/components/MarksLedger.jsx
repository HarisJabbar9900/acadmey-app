        ...
        {classTests.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {classTests.map((test) => {
              ...
              // Calculate compact test evaluation stats
              const evaluatedScores = testClassStudents
                .map(s => (test.scores && test.scores[s.id] !== undefined ? Number(test.scores[s.id]) : null))
                .filter(sc => sc !== null && !isNaN(sc));
              const evaluatedCount = evaluatedScores.length;
              const totalCount = testClassStudents.length;

              let avgPct = 0;
              let highestScore = 0;
              if (evaluatedScores.length > 0 && Number(test.maxMarks) > 0) {
                const sum = evaluatedScores.reduce((acc, curr) => acc + curr, 0);
                avgPct = Math.round((sum / (evaluatedScores.length * Number(test.maxMarks))) * 100);
                highestScore = Math.max(...evaluatedScores);
              }
              ...
              <div key={test.id} className="bg-white dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800 rounded-2xl p-5 hover:border-indigo-500/40 shadow-sm hover:shadow-md transition-all flex flex-col justify-between">
                <div>
                  ...
                  {/* 📊 Compact Stats & Accordion Toggle Row */}
                  <div className="mt-3.5 pt-3 border-t border-slate-200/80 dark:border-slate-800/60 flex flex-wrap items-center justify-between gap-2">
                    <div className="flex flex-wrap items-center gap-1.5 sm:gap-2">
                      <span className="px-2.5 py-1 bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 rounded-lg font-mono text-[11px] font-bold shadow-xs">
                        👥 {evaluatedCount}/{totalCount} Evaluated
                      </span>
                      {evaluatedCount > 0 && (
                        <span className="px-2.5 py-1 bg-emerald-50 dark:bg-emerald-500/10 text-emerald-700 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-500/20 rounded-lg font-mono text-[11px] font-bold shadow-xs">
                          Avg: {avgPct}%
                        </span>
                      )}
                      {evaluatedCount > 0 && (
                        <span className="hidden sm:inline-block px-2.5 py-1 bg-amber-50 dark:bg-amber-500/10 text-amber-800 dark:text-amber-400 border border-amber-200 dark:border-amber-500/20 rounded-lg font-mono text-[11px] font-bold shadow-xs">
                          Top: {highestScore}/{test.maxMarks}
                        </span>
                      )}
                    </div>

                    {/* Expand/Collapse Button (Collapsed by Default) */}
                    <button
                      type="button"
                      onClick={() => toggleExpandTest(test.id)}
                      className="px-3 py-1.5 bg-indigo-50 hover:bg-indigo-100 dark:bg-indigo-500/10 dark:hover:bg-indigo-500/20 text-indigo-700 dark:text-indigo-300 border border-indigo-200 dark:border-indigo-500/30 rounded-xl text-xs font-bold flex items-center gap-1.5 transition-all cursor-pointer shadow-xs active:scale-95"
                    >
                      {isExpanded ? (
                        <>
                          <EyeOff className="w-3.5 h-3.5 text-indigo-600 dark:text-indigo-400" />
                          <span>Hide Student Marks</span>
                          <ChevronUp className="w-3 h-3 text-indigo-500" />
                        </>
                      ) : (
                        <>
                          <Eye className="w-3.5 h-3.5 text-indigo-600 dark:text-indigo-400" />
                          <span>View Marks ({totalCount})</span>
                ...
                {/* Score Summary breakdown (Hidden by default, only shown when user taps "View Marks") */}
                {isExpanded && (
                  <div className="mt-4 pt-3 border-t border-slate-200/80 dark:border-slate-800/80 bg-slate-50 dark:bg-slate-950/40 rounded-xl p-3 animate-fade-in">
                    <div className="flex items-center justify-between text-xs font-bold text-slate-700 dark:text-slate-300 mb-2">
                      <span>Student Marks List ({testClassStudents.length}):</span>
                      <span className="text-[10px] text-slate-400 hidden sm:inline">Click 💬 to send result via WhatsApp</span>
                    </div>
