        ...
        <div className="space-y-4">
          {pinnedAnnouncements.map((notice) => {
            ...
            return (
              <div 
                key={notice.id}
                className="relative rounded-3xl border-2 border-amber-500/50 bg-gradient-to-br from-amber-500/5 via-white dark:via-slate-900 to-amber-500/10 p-4 sm:p-5 shadow-lg shadow-amber-500/5 overflow-hidden transition-all"
              >
                ...
                <div className="relative z-10 space-y-4">
                  ...
                  {/* Header Ribbon */}
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2.5 pb-3 border-b border-amber-500/20">
                    <div className="flex flex-wrap items-center gap-2">
                      {isRecent && (
                        <span className="px-2 py-0.5 rounded-lg text-[10px] font-black bg-gradient-to-r from-rose-600 to-amber-600 text-white shadow-sm flex items-center gap-1.5">
                    ...
                    <div className="flex items-center gap-2 self-end sm:self-auto print:hidden">
                      <button
                        ...
                        className={`px-3 py-1 rounded-xl text-xs font-bold flex items-center gap-1.5 transition-all cursor-pointer shadow-xs active:scale-95 ${
                          isCopied
                      ...
                      {isAdminLoggedIn && (
                        <button
                          ...
                          className="p-1 bg-rose-500/10 hover:bg-rose-500/20 text-rose-600 dark:text-rose-400 border border-rose-500/30 rounded-lg transition-all cursor-pointer shadow-xs"
                          title="Delete Notice"
                        >
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="flex items-center justify-between pb-1.5 border-b border-amber-500/20" dir="rtl">
                      <div className="flex items-center gap-2">
                        <span className="w-2 h-2 rounded-full bg-emerald-500 inline-block animate-pulse" />
                        <span 
                          style={JAMEEL_FONT} 
                          className="text-xs sm:text-sm font-bold text-amber-700 dark:text-amber-400"
                        >
                    ...
                    {/* Urdu Title (Jameel Noori Nastaleeq - with ample line-height) */}
                    <h3 
                      style={JAMEEL_FONT} 
                      dir="rtl"
                      className="text-xl sm:text-2xl font-bold text-slate-900 dark:text-amber-400 leading-[2.3] text-right overflow-visible pt-0.5"
                    >
                      {notice.title}
                    </h3>

                    {/* Urdu Lead Statement (Jameel Noori Nastaleeq - compact padding) */}
                    {parsed.urduLead && (
                      <div 
                        dir="rtl" 
                        style={JAMEEL_FONT} 
                        className="text-sm sm:text-base text-slate-800 dark:text-slate-200 leading-[2.4] text-right bg-amber-500/10 dark:bg-amber-500/5 p-3 sm:p-3.5 rounded-xl border border-amber-500/20 shadow-xs overflow-visible"
                      >
                        {parsed.urduLead}
                      </div>
                    )}

                    {/* SEPARATED NUMBERED POINTS (COMPACT, CLOSE & NO CUT-OFF) */}
                    {parsed.urduPoints.length > 0 && (
                      <div className="space-y-2 pt-0.5">
                        <div className="flex items-center justify-between" dir="rtl">
                          <span 
                            style={JAMEEL_FONT} 
                            className="text-xs sm:text-sm font-bold text-slate-800 dark:text-slate-200 flex items-center gap-1.5 overflow-visible"
                          >
                            📌 اہم ہدایات و ضوابط (نمبر وار):
                          </span>
                        ...
                        {/* Compact, close-knit points: inline title and text with ample line-height so no letters cut off */}
                        <div className="space-y-2">
                          {parsed.urduPoints.map((pt, idx) => (
                            <div 
                              key={idx}
                              dir="rtl"
                              className="p-2.5 sm:p-3 rounded-xl bg-amber-500/5 dark:bg-slate-800/70 border border-amber-500/25 hover:border-amber-500/40 transition-all flex items-start gap-2.5 shadow-xs overflow-visible"
                            >
                              {/* Compact Number Badge */}
                              <span className="w-6 h-6 rounded-lg bg-gradient-to-br from-amber-500 to-amber-600 text-slate-950 font-black text-xs flex items-center justify-center font-mono shrink-0 mt-1 shadow-xs">
                                {pt.number || (idx + 1)}
                              </span>

                              {/* Numbered point text: title + text inline, plenty of vertical room */}
                              <div 
                                style={JAMEEL_FONT} 
                                className="text-right flex-1 overflow-visible leading-[2.4] pt-0.5"
                              >
                                {pt.title && (
                                  <span className="font-bold text-sm sm:text-base text-amber-700 dark:text-amber-300 ml-1.5 inline">
                                    {pt.title}:
                                  </span>
                                )}
                                <span className="text-xs sm:text-sm text-slate-800 dark:text-slate-200 inline">
                                  {pt.text}
                                </span>
                              </div>
                            </div>
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 pt-2.5 border-t border-amber-500/20 text-xs">
                      <div className="flex items-center gap-3 text-slate-500 dark:text-slate-400 font-mono text-[11px]">
                        ...
                        {notice.startDate && (
                          <span className="px-2 py-0.5 rounded-md bg-amber-500/10 text-amber-700 dark:text-amber-400 border border-amber-500/30 font-bold text-[10px]">
                            آغاز ٹیسٹ: {notice.startDate}
                          </span>
                        )}
                      </div>

                      <div className="flex items-center gap-1.5 self-end sm:self-auto" dir="rtl">
                        <span className="w-2 h-2 rounded-full bg-emerald-500 inline-block animate-pulse" />
                        <span 
                          style={JAMEEL_FONT} 
                          className="text-xs sm:text-sm font-bold text-slate-800 dark:text-slate-200 overflow-visible"
                        >
                  ...
                  {/* ========================================================= */}
                  {/* 2. COMPLETELY SEPARATED ENGLISH SECTION (COMPACT & SLEEK) */}
                  {/* ========================================================= */}
                  {(parsed.englishTitle || parsed.englishNote || parsed.englishPoints.length > 0) && (
                    <div className="mt-3.5 pt-2.5 border-t-2 border-dashed border-slate-200 dark:border-slate-800 space-y-2" dir="ltr">
                      {/* English Header */}
                      <div className="flex items-center justify-between pb-1 border-b border-indigo-500/20">
                        <div className="flex items-center gap-1.5">
                          ...
                          <span className="text-[10px] font-black uppercase tracking-wider text-indigo-600 dark:text-indigo-400 font-mono">
                            English Notice (Official Summary)
                          </span>
                        </div>
                        <span className="text-[9px] font-mono px-1.5 py-0.5 rounded bg-indigo-500/10 text-indigo-700 dark:text-indigo-300 font-bold border border-indigo-500/20">
                          ENGLISH
                        </span>
                      </div>
                      ...
                      {parsed.englishNote && (
                        <div className="bg-indigo-50/60 dark:bg-indigo-950/20 border border-indigo-200/60 dark:border-indigo-800/40 rounded-xl p-2.5 text-xs text-slate-700 dark:text-slate-300 leading-relaxed">
                          {parsed.englishNote}
                        </div>
                      )}

                      {/* English Numbered Points (Compact) */}
                      {parsed.englishPoints.length > 0 && (
                        <div className="space-y-1.5">
                          {parsed.englishPoints.map((pt, idx) => (
                            <div 
                              key={idx}
                              className="p-2 sm:p-2.5 rounded-lg bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 flex items-start gap-2 shadow-2xs text-xs"
                            >
                              <span className="w-4 h-4 rounded-md bg-indigo-500/15 text-indigo-600 dark:text-indigo-400 font-black text-[10px] flex items-center justify-center font-mono shrink-0 mt-0.5">
                                {pt.number || (idx + 1)}
                              </span>
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-1 pt-1.5 border-t border-slate-200 dark:border-slate-800 text-[10px] font-mono text-slate-500 dark:text-slate-400">
                        <span>Authorized: <strong>Principal & Admin, Al-Zia Science Academy</strong></span>
                        <span>Commencement: {notice.startDate || '1 Oct 2026'}</span>
                      </div>
                    </div>
                  )}
                    ...
                    {parsed.urduPoints.length > 0 && (
                      <div className="space-y-2 pt-1" dir="rtl">
                        {parsed.urduPoints.map((pt, idx) => (
                          <div 
                            key={idx}
                            className="p-2 sm:p-2.5 rounded-xl bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700/80 flex items-start gap-2 text-right overflow-visible"
                          >
                            <span className="w-5 h-5 rounded-md bg-amber-500/20 text-amber-700 dark:text-amber-400 font-bold text-[11px] flex items-center justify-center font-mono shrink-0 mt-0.5">
                              {pt.number || (idx + 1)}
                            </span>
                            <div 
                              style={JAMEEL_FONT} 
                            ...
                            >
                              {pt.title && (
                                <strong className="text-xs font-bold text-amber-700 dark:text-amber-300 ml-1.5 inline">
                                  {pt.title}:
                                </strong>
                              )}
                              <span className="text-xs text-slate-700 dark:text-slate-300 inline">
                                {pt.text}
                              </span>
                            </div>
                          </div>
