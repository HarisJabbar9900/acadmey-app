...
export default function StaffInfo({ faculty = [], isAdminLoggedIn, onUpdateFaculty }) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white/95 dark:bg-slate-900/90 border border-slate-200/80 dark:border-slate-800 rounded-2xl p-4 sm:p-5 shadow-xs flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <div className="inline-flex items-center gap-1.5 text-xs font-bold uppercase tracking-wider text-indigo-600 dark:text-indigo-400 mb-1">
            ...
            <span>Academic Leadership & Faculty</span>
          </div>
          <h2 className="text-xl sm:text-2xl font-black text-slate-900 dark:text-white flex items-center gap-2 tracking-tight">
            <span>Teaching Faculty & Staff Directory</span>
          </h2>
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
            Directory of subject specialists, board examiners, and educators.
          </p>
        </div>

        <div className="flex flex-wrap items-center gap-2.5">
          {/* Staff Counter Badge */}
          <div className="px-3 py-1.5 bg-slate-100 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300 rounded-xl text-xs font-bold font-mono flex items-center gap-1.5 shadow-xs">
            <Users className="w-3.5 h-3.5 text-indigo-500" />
            <span>{currentFacultyList.length} Faculty</span>
          </div>
          ...
          {isAdminLoggedIn ? (
            <button
              ...
              className="px-4 py-2 bg-gradient-to-r from-indigo-600 to-indigo-700 hover:from-indigo-500 hover:to-indigo-600 active:scale-95 text-white rounded-xl text-xs sm:text-sm font-bold flex items-center gap-1.5 shadow-xs transition-all cursor-pointer whitespace-nowrap"
            >
              <PlusCircle className="w-4 h-4" />
              <span>+ Add Faculty</span>
            </button>
          ) : (
            <div className="px-3 py-1.5 bg-slate-100 dark:bg-slate-800/80 text-slate-600 dark:text-slate-400 rounded-xl text-xs font-semibold flex items-center gap-1.5 border border-slate-200 dark:border-slate-700 shadow-xs">
      ...
      {/* 📞 Official Academic Inquiries & Administration: Sir Zia */}
      <div className="bg-gradient-to-r from-emerald-600 via-emerald-700 to-teal-700 text-white rounded-2xl p-4 sm:p-5 shadow-md flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex items-center gap-3.5">
          <div className="w-11 h-11 rounded-2xl bg-white/20 backdrop-blur-md flex items-center justify-center text-white shrink-0 shadow-xs border border-white/20">
            <Phone className="w-5 h-5" />
          </div>
          <div>
            <div className="flex items-center gap-2 flex-wrap">
              <span className="text-[10px] font-black uppercase tracking-wider text-emerald-200 bg-white/10 px-2 py-0.5 rounded-md">
        ...
        <div className="flex items-center gap-2.5 shrink-0">
          <a
            href={`https://wa.me/${SIR_ZIA_WA}?text=${encodeURIComponent("Assalamu Alaikum Sir Zia! I am a student of Al-Zia Science Academy and have an academic inquiry.")}`}
            target="_blank"
            rel="noreferrer"
            className="px-4 py-2.5 bg-white text-emerald-800 hover:bg-emerald-50 rounded-xl text-xs font-black flex items-center gap-2 shadow-sm transition-all cursor-pointer active:scale-95 whitespace-nowrap"
          >
            <MessageSquare className="w-4 h-4 text-emerald-600 fill-emerald-600" />
            <span>WhatsApp Sir Zia</span>
          </a>

          <a
            href={`tel:${SIR_ZIA_TEL}`}
            className="px-3.5 py-2.5 bg-white/15 hover:bg-white/25 text-white border border-white/20 rounded-xl text-xs font-bold flex items-center gap-1.5 transition-all cursor-pointer whitespace-nowrap"
          >
      ...
      {currentFacultyList.length === 0 ? (
        <div className="glass-panel p-10 sm:p-14 text-center rounded-3xl border border-dashed border-slate-300 dark:border-slate-800 space-y-4 max-w-lg mx-auto my-4">
          <div className="p-4 bg-indigo-50 dark:bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 rounded-2xl w-16 h-16 mx-auto flex items-center justify-center border border-indigo-200 dark:border-indigo-500/20 shadow-xs">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-1.5 max-w-md mx-auto leading-relaxed">
              Dummy staff records have been removed. {isAdminLoggedIn ? 'Click "+ Add Faculty Member" below to start adding your real academy teachers, their subjects, degrees, and qualifications.' : 'Faculty details will appear here once added by the administration.'}
            </p>
          </div>
          {isAdminLoggedIn && (
            <button
              type="button"
              onClick={handleOpenAddModal}
              className="px-5 py-2.5 bg-gradient-to-r from-indigo-600 to-indigo-700 hover:from-indigo-500 hover:to-indigo-600 text-white rounded-xl text-xs sm:text-sm font-bold shadow-lg shadow-indigo-600/30 transition-all cursor-pointer inline-flex items-center gap-2 active:scale-95"
            >
              <PlusCircle className="w-4 h-4" />
